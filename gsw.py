...
class GSW():
    ...
    def gsw(self,X,Y,U,theta=None):
        ...
        #return torch.sum((Xslices - Yslices)*((Yslices - Xslices <= 0).float() - 0.1))

        Xslices_sorted=torch.sort(Xslices,dim=0)[0]
        Yslices_sorted=torch.sort(Yslices,dim=0)[0]
        return torch.mean((Xslices_sorted-Yslices_sorted)*((Yslices_sorted - Xslices_sorted <= 0).float() - U))

    def gsw_q1(self,X,Y,tau,theta=None):
        '''
        Calculates GSW between two empirical distributions.
        Note that the number of samples is assumed to be equal
        (This is however not necessary and could be easily extended
        ...
        for i in range(tau.shape[-1]):
            u = (tau[:, i].unsqueeze(-1) - (Yslices_sorted - Xslices_sorted <= 0).float())
            loss += torch.sum((Xslices_sorted-Yslices_sorted)*u)
        return loss
