...
def data_loader(x, y, train_split, test_split, batch_size):
    ...
    # Convert to Tensor #
    train_set = TensorDataset(torch.from_numpy(train_seq).float().cuda(), torch.from_numpy(train_label).float().cuda())
    val_set = TensorDataset(torch.from_numpy(val_seq).float().cuda(), torch.from_numpy(val_label).float().cuda())
    test_set = TensorDataset(torch.from_numpy(test_seq).float().cuda(), torch.from_numpy(test_label).float().cuda())
